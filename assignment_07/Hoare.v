...
Add LoadPath "F:\Dropbox\ITU\SDT\PLS\exercises\assignment_07".
...
(** Which of the following Hoare triples are _valid_ -- i.e., the
   ...
   4) {{X = 2 /\ X = 3}} X ::= 5 {{X = 0}} valid (absurd)

   5) {{True}} SKIP {{False}} invalid

   6) {{False}} SKIP {{True}} valid (absurd)

   7) {{True}} WHILE True DO SKIP END {{False}} valid (absurd)
...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c H st st' H0 H1. apply H.
Qed.
...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c H st st' H0 H1. unfold "~" in H. contradiction (H st H1).
Qed.
...
Theorem hoare_asgn : forall Q X a,
  {{Q [X |-> a]}} (X ::= a) {{Q}}.
Proof.
  intros Q X a st st' H1 H2.
  unfold assn_sub in H2. inversion H1. subst.
  apply H2.
Qed.
...
(**
what if a = Plus (AId X) (ANum 1), then the postcondition will say that
X = X + 1 which is a recursive equation with no solution.
**)

Theorem hoare_asgn_wrong : (forall X a,
  {{fun _ => True}} (X ::= a) {{fun st => st X = (aeval st a)}}) -> False.
Proof.
  intros H. unfold hoare_triple in H.
  remember (t_update empty_state X 5) as st.
  remember (t_update st X 6) as st'.
  remember (APlus (AId X) (ANum 1)) as a.
  specialize (H X a st st').
  subst. simpl in H.
  rewrite t_update_eq in H. assert (6 = 7 -> False) by omega.
  apply H0. apply H.
...
Proof.
  ...
  split.
  - assert (st = t_update st' X m). apply functional_extensionality.
    + intros. unfold t_update. destruct (beq_id X x) eqn:Hbeq.
      * rewrite beq_id_true_iff in Hbeq. subst. reflexivity.
      * inversion H. unfold t_update. destruct (beq_id X x); [| omega ].
        subst. inversion Hbeq.
...
Proof.
  ...
  exists (st X). split.
  - assert (st = t_update st' X (st X)). apply functional_extensionality. intros.
    + unfold t_update. destruct (beq_id X x) eqn:Heq.
      * rewrite beq_id_true_iff in Heq. rewrite Heq. reflexivity.
      * inversion H. subst. unfold t_update. rewrite Heq. reflexivity.
...
Theorem hoare_consequence_pre : forall (P P' Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P P' Q c H H0 st st' H1 H2.
  apply (H st st' H1). apply (H0 _ H2).
Qed.
...
Theorem hoare_consequence_post : forall (P Q Q' : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q Q' c H1 H2 st st' H3 H4.
  apply H2. apply (H1 st st' H3 H4).
Qed.
...
Proof.
  eapply hoare_consequence_pre.
  - apply hoare_asgn.
  - unfold "->>". intros. unfold assn_sub. simpl. rewrite t_update_eq. assumption.
Qed.
(* here with apply 
  apply hoare_consequence_pre
...
Example ass_sub_ex2' :
  {{ fun st => 0 <= 3 /\ 3 <= 5 }}  X ::= ANum 3  {{ fun st => 0 <= st X /\ st X <= 5 }}.
Proof.
  apply hoare_consequence_pre
...
Proof.
  ...
  - apply hoare_consequence_pre with ((fun st => st X = 1 /\ st Y = 2) [Y |-> ANum 2]).
    + apply hoare_asgn.
    + unfold "->>", assn_sub. intros. simpl. split.
      * unfold t_update. destruct (beq_id Y X) eqn:Heq; [inversion Heq | assumption].
      * apply t_update_eq.
  - apply hoare_consequence_pre with ((fun st => st X = 1) [X |-> ANum 1]).
    + apply hoare_asgn.
    + unfold "->>", assn_sub. intros. simpl. apply t_update_eq.
Qed.
(* a lot easier with eapply
  eapply hoare_seq; [eapply hoare_asgn |].
  eapply hoare_consequence_pre; [apply hoare_asgn |].
  unfold "->>", assn_sub. intros. simpl. split.
...
Proof.
  ...
  - eapply hoare_consequence_pre; [apply hoare_asgn |].
    unfold "->>", assn_sub. intros. simpl.
    apply H. (* why is this possible!?*)
Qed.
(** [] *)
...
(** Because a can depend on the state it is evaluated in, and since
 it is another state we're in when we assign Y we cannot say that a
 evals to the same n again
 **)
(** [] *)
...
Proof.
  apply hoare_if; (eapply hoare_consequence_pre; [apply hoare_asgn |];
                  unfold bassn, assn_sub, "->>"; intros; destruct H).
  - simpl. rewrite t_update_eq. rewrite t_update_neq.
    + rewrite Nat.add_sub_assoc. rewrite minus_plus; reflexivity.
      simpl in H0. apply (leb_complete _ _ H0).
    + unfold not. intros. inversion H1.
  - rewrite ?t_update_eq. simpl. rewrite 2!t_update_neq;
                                   [reflexivity | |];
                                   unfold not; intros; inversion H1.
Qed. 
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_If1False : forall (st st' : state) (b1 : bexp) (c1 : com),
                beval st b1 = false ->
                (IF1 b1 THEN c1 FI) / st \\ st
...
Theorem hoare_if1 : forall P Q b c1,
  {{fun st => P st /\ bassn b st}} c1 {{Q}} ->
  ((fun st => P st /\ ~(bassn b st)) ->> Q) ->
  {{P}} (IF1 b THEN c1 FI) {{Q}}.
  Proof.
    intros P Q b c1 H1 H2 st st' H3 HP.
    inversion H3; subst.
    - apply (H1 st st' H7). split; assumption.
    - apply H2. split; [assumption|]. congruence.
  Qed.
...
Lemma hoare_if1_good :
  ...
  {{ fun st => st X = st Z }}.
Proof.
  intros st st' H1 H2. inversion H1; subst.
  - inversion H6. subst. simpl.
    rewrite H2. reflexivity.
  - simpl in *. rewrite negb_false_iff in H5.
    pose proof (beq_nat_true _ _ H5).
    rewrite H in H2. rewrite plus_0_r in H2. apply H2.
  Qed.
...
Proof.
  unfold ex1_repeat. constructor; [reflexivity |].
...
Lemma hoare_repeat : forall P Q b c,
  {{P}} c {{Q}} -> {{fun st => Q st /\ ~(bassn b st)}} c {{Q}} ->
  {{P}} REPEAT c UNTIL b END {{fun st => Q st /\ bassn b st}}.
Proof.
  intros P Q b c H1 H2 st st' Hc HP.
  unfold hoare_triple in *.
  remember (REPEAT c UNTIL b END) as Hrem eqn:HremEq.
  induction Hc; intros; inversion HremEq; subst.
  - split; [apply (H1 _ _ Hc HP) | apply (bexp_eval_true _ _ H)].
  Abort.
...
Lemma hoare_repeat' : forall P Q b c,
    ...
    {{P}} REPEAT c UNTIL b END {{fun st => Q st /\ bassn b st}}.
Proof.
  intros P Q b c Hc HInv st st' He HP.
  unfold "->>" in *.
  remember (REPEAT c UNTIL b END) as wcom eqn:Heqwcom.
  unfold hoare_triple in *.
  induction He; inversion Heqwcom; subst; clear Heqwcom.
  - split; [apply (Hc _ _ He HP)|].
    apply (bexp_eval_true _ _ H).
  - apply IHHe2; [reflexivity |]. apply HInv. split; [apply (Hc _ _ He1 HP) |].
    apply (bexp_eval_false _ _ H).
Qed.
...
Coq proof below.
I'm pretty sure it could've been written simpler with more eapply and hoare_asgn,
but it just confused me, so I brute-forced it!
**)

Theorem hoare_repeat_test :
  {{ fun st => st X > 0 }}
    REPEAT
    Y ::= AId X;;
    X ::= AMinus (AId X) (ANum 1)
...
Proof.
  apply hoare_consequence_post with (Q' := fun st => st Y > 0 /\ bassn (BEq (AId X) (ANum 0)) st).
  - apply hoare_repeat.
    + apply hoare_seq with (Q := fun st => st Y > 0).
      * unfold hoare_triple. intros. inversion H. subst. simpl in *. rewrite t_update_neq.
        assumption. unfold not. intros. inversion H1.
      * unfold hoare_triple. intros. inversion H. subst. simpl in *. rewrite t_update_eq. assumption.
    + unfold "->>". intros. destruct H. unfold not in H0. simpl in H0.
      unfold bassn in H0. simpl in H0. rewrite beq_nat_true_iff in H0.
      destruct (gt_0_eq (st X)); [assumption |].
      symmetry in H1. contradiction (H0 H1).
  - unfold "->>". unfold bassn. simpl. intros. rewrite beq_nat_true_iff in H.
...
(** this basically says that "Q doesn't care what value X is" **)
...
Proof.
  intros Q X st st' c H. unfold havoc_pre in H. inversion c. subst. apply H.
Qed.
