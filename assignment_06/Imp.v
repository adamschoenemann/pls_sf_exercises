...
Proof.
  induction b; simpl; try (rewrite ?optimize_0plus_sound''; reflexivity).
...
*)
Fixpoint optimized_aeval (a : aexp) : aexp :=
  match a with
  | ANum n =>
      ANum n
  | APlus (ANum 0) e2 =>
  ...
  | APlus e1 (ANum 0) =>
      optimized_aeval e1
  | APlus e1 e2 =>
      APlus (optimized_aeval e1) (optimized_aeval e2)
  | AMinus e1 (ANum 0) =>
     optimized_aeval e1
  | AMinus e1 e2 =>
      AMinus (optimized_aeval e1) (optimized_aeval e2)
  | AMult (ANum 1) e2 =>
      optimized_aeval e2
  | AMult e1 (ANum 1) =>
      optimized_aeval e1
  | AMult e1 e2 =>
      AMult (optimized_aeval e1) (optimized_aeval e2)
  end.
...
| E_BLe : forall a1 a2 n1 n2,
    aevalR a1 n1 -> aevalR a2 n2 -> bevalR (BLe a1 a2) (leb n1 n2)
...
Proof.
  split.
  - intros H. induction H; simpl; rewrite ?aeval_iff_aevalR' in *; subst; reflexivity.
  - generalize dependent b.
    induction e; simpl; intros; subst;
      (constructor; try (rewrite ?aeval_iff_aevalR'; reflexivity)).
...
Proof.
  remember (t_update empty_state X 0) as s1.
  remember (t_update s1 Y 1) as s2.
  remember (t_update s2 Z 2) as s3.
  apply E_Seq with (s1); [rewrite Heqs1; constructor; reflexivity |].
  apply E_Seq with (s2); [rewrite Heqs2 | rewrite Heqs3] ; constructor; reflexivity.
Qed.
...
Proof.
  remember (t_update empty_state X 2) as s1.
  remember (t_update s1 Y 0) as s2.
  remember (t_update s2 Y 2) as s3.
  remember (t_update s3 X 1) as s4.
  remember (t_update s4 Y 3) as s5.
  remember (t_update s5 X 0) as s6.
  apply E_Seq with (st := s1) (st' := s2) (st'' := s6);
  [rewrite Heqs2; constructor; reflexivity |].
  apply E_WhileLoop with (st := s2) (st' := s4) (st'' := s6);
  [subst; reflexivity | |].
  - apply E_Seq with s3; subst; constructor; reflexivity.
  - apply E_WhileLoop with (st := s4) (st' := s6); [subst; reflexivity | |].
    + apply E_Seq with s5; subst; constructor; reflexivity.
...
Lemma XtimesYinZ_lem : forall x y st,
    XtimesYinZ / (t_update (t_update empty_state X x) Y y) \\ st ->
    st Z = x * y.
Proof.
  induction x; intros; simpl; inversion H; subst; reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - subst. apply (IHcontra2 Heqloopdef).
Qed.
...
| NW_Ass  : forall x y, no_whilesR (x ::= y)
...
Proof.
  ...
  - exists (t_update st x (aeval st y)). constructor. reflexivity.
  - destruct (IHno_whilesR1 st) as [st'], (IHno_whilesR2 st') as [st''].
    exists st''. apply (E_Seq _ _ st st' st'' H1 H2).
  - destruct (beval st b) eqn:Hb;
    [destruct (IHno_whilesR1 st) as [st'] | destruct (IHno_whilesR2 st) as [st']];
    exists st'; constructor; assumption; assumption.
Qed.
...
Definition stack_op (op:nat -> nat -> nat) (stack: list nat) : list nat :=
  match stack with
  | (h :: h' :: t) => op h' h :: t
  | _ => stack
  end.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | x :: xs => match x with
    ...
    | SLoad i => s_execute st (st i :: stack) xs
    | SPlus   => s_execute st (stack_op plus  stack) xs
    | SMinus  => s_execute st (stack_op minus stack) xs
    | SMult   => s_execute st (stack_op mult  stack) xs
              end
  end.
...
Example s_compile_ex01 :
  s_compile (APlus (ANum 1) (ANum 5)) = [SPush 1; SPush 5; SPlus].
Proof.
  reflexivity.
Qed.
...
(* (2*3)+(3*(4-2)) *)
Definition aexpr_ex01 := 
  APlus (AMult (ANum 2) (ANum 3))
        (AMult (ANum 3) (AMinus (ANum 4) (ANum 2))).

(*      2 3 * 3 4 2 - * +     *)
Example s_compile2 :
  s_compile aexpr_ex01
  = [SPush 2; SPush 3; SMult; SPush 3; SPush 4; SPush 2;
       SMinus; SMult; SPlus
    ].
Proof.
  reflexivity.
...
Example s_execute3 :
  s_execute empty_state [] (s_compile aexpr_ex01) = [12].
Proof.
  reflexivity.
Qed.
...
Lemma s_execute_app_seq : forall l1 l2 st stk,
...
Lemma s_compile_not_nil : forall e, ~(s_compile e = []).
Proof.
  unfold not. induction e; intros; simpl in *; inversion H;
  symmetry in H; rewrite app_assoc in H;
  apply (app_cons_not_nil _ _ _ H).
Qed.

Lemma s_compile_correct_lem : forall (e : aexp) (st : state) stk,
    s_execute st stk (s_compile e) = aeval st e :: stk.
Proof.
  induction e; intros; simpl; try reflexivity;
  rewrite 2!s_execute_app_seq; rewrite IHe1, IHe2; reflexivity.
Qed.
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Seq_Cont : forall c1 c2 st st' st'' s,
      ...
      c2 / st' \\ s / st'' ->
      (c1 ;; c2) / st \\ s / st''
  | E_Seq_Break : forall c1 c2 st st' st'' s,
      c1 / st  \\ SBreak / st' ->
      c2 / st' \\ s / st'' ->
      (c1 ;; c2) / st \\ SBreak / st'
...
Theorem break_ignore : forall c st st' s,
     ...
     st = st'.
Proof.
  intros. inversion H; inversion H2. reflexivity.
Qed.
...
Theorem while_continue : forall b c st st' s,
  ...
  s = SContinue.
Proof.
  intros. inversion H; [reflexivity | reflexivity | reflexivity].
Qed.
...
Proof.
  intros. remember (WHILE b DO c END). induction H; try inversion Heqc0.
  - subst. rewrite H in H0. inversion H0.
  - rewrite H4 in H1. exists st. assumption.
  - apply IHceval2.
    + subst. reflexivity.
    + assumption.
Qed.
...
Proof.
  ...
  induction H; intros.
  - inversion H0. split; reflexivity.
  - inversion H0. split; reflexivity.
  - inversion H0. subst. split; reflexivity.
  - inversion H1; subst; destruct (IHceval1 _ _ H4).
    + apply IHceval2.
  ...
  - inversion H1; subst; destruct (IHceval1 _ _ H4).
    + inversion H3.
    + split; assumption.
  - inversion H1; subst; [apply (IHceval _ _ H9) | ].
    rewrite H8 in H. inversion H.
  - inversion H1; subst; [ | apply (IHceval _ _ H9)].
    rewrite H8 in H. inversion H.
  - inversion H0; [split; reflexivity | |]; rewrite H3 in H; inversion H.
  - inversion H1; subst.
    + rewrite H7 in H. inversion H.
    + destruct (IHceval _ _ H8). split; [assumption | reflexivity].
    + destruct (IHceval _ _ H5). inversion H3.
  - inversion H2; subst.
    + rewrite H8 in H. inversion H.
    + destruct (IHceval1 _ _ H9). inversion H4.
    + destruct (IHceval1 _ _ H6). subst. destruct (IHceval2 _ _ H10).
...
Fixpoint beval_alt (st : state) (b : bexp) : bool :=
  match b with
  | BTrue       => true
  | BFalse      => false
  | BEq a1 a2   => beq_nat (aeval st a1) (aeval st a2)
...
Theorem beval_alt_beval : forall st b, beval_alt st b = beval st b.
Proof.
  intros. generalize dependent st. induction b; intros; reflexivity.
Qed.
