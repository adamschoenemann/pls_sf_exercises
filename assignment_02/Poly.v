...
- not well-typed (needs a type application)
- well-typed
- well-typed
- well-typed
- well-typed
- not well-typed (expected a bool).
- well-typed
*)
...
Check combine.
(*
  Check combine. ==>  (list ?X) -> (list ?Y) -> list (?X * ?Y).
    where ?X : [ |- Type]
          ?Y : [ |- Type].
  Compute (combine [1;2] [false;false;true;true]). ==>
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => evenb x && (blt_nat 7 x)) l.
...
Lemma map_dist_app : forall (X Y : Type) (l1 l2 : list X) (f : X -> Y),
...
Proof.
  induction l.
  - reflexivity.
  - simpl. rewrite map_dist_app. rewrite IHl. reflexivity.
Qed.
...
(* Yes, plenty of times. For example, length = fold (fun (x y) => y + 1)
   has X = whatever and Y = nat
 *)
...
Since nth_error zero-indexes the list l, then there cannot be an element
at index n, and therefore nth_error must return null if n is the length
of l.
*)
(** [] *)
...
Definition mult (n m : nat) : nat :=
  fun (X : Type) (f : X -> X) (x : X) => n X (fun z => m X f z) x.
...
(*
We have to iterate n with itself m times
So, n is a function that given a type and a function f and a value x will iterate
f over x n times.

*)
Definition exp (n m : nat) : nat :=
  fun (X : Type) (f : X -> X) (x : X) =>
    m (X -> X) (fun z y => n X z y) f x. 
...
Definition church2nat (n : Church.nat) : nat :=
  n nat S O.

Compute church2nat (Church.three).
