...
Proof.
  ...
  - intros. induction H.
    + apply ev'_0.
    + assert (S (S n) = 2 + n) by reflexivity. rewrite H0.
      apply ev'_sum.
      * apply ev'_2.
      * apply IHev.
Qed.
...
Lemma double_n_ev : forall n, ev (n + n).
Proof.
  intros. induction n.
  - simpl. apply ev_0.
  - simpl. rewrite plus_n_Sm. rewrite plus_comm. simpl. apply (ev_SS _ IHn).
Qed.
...
Proof.
  intros. pose proof (ev_sum _ _ H H0). assert (ev (n + n + (m + p))).
  - rewrite plus_assoc. rewrite plus_comm. rewrite <- plus_assoc. 
    rewrite plus_comm. replace (n + m) with (m + n). rewrite plus_assoc. 
    rewrite <- plus_assoc. apply H1. apply plus_comm.
  - apply (ev_ev__ev _ _ H2). apply double_n_ev.
Qed.
...
Theorem Sn_le_Sm__n_le_m : forall n m,
  S n <= S m -> n <= m.
Proof.
  intros. inversion H. apply (le_n m). apply (le_trans _ (S n)).
  - apply (le_S _ _ (le_n n)).
  - assumption.
Qed.
...
Proof.
  ...
  - induction H.
    + apply n_le_m__Sn_le_Sm. apply (le_plus_l n1 n2).
    + apply n_le_m__Sn_le_Sm. apply le_S in IHle. apply (Sn_le_Sm__n_le_m _ _ IHle).
  - induction H.
    + apply n_le_m__Sn_le_Sm. rewrite plus_comm. apply (le_plus_l n2 n1).
    + apply n_le_m__Sn_le_Sm. apply le_S in IHle. apply (Sn_le_Sm__n_le_m _ _ IHle).
Qed.
...
Proof.
  induction n.
  - intros. rewrite <- plus_O_n. apply le_plus_l.
  - intros. destruct m.
    + simpl in H. inversion H.
    + simpl in H. apply n_le_m__Sn_le_Sm. apply (IHn _ H).
Qed.
...
Proof.
  intros. apply leb_correct. apply (le_trans n m o).
  - apply (leb_complete _ _ H).
  - apply (leb_complete _ _ H0).
Qed.
...
(** - Which of the following propositions are provable?
    ...
    - If we dropped constructor [c5] from the definition of [R],
      ...
      Not really, since we can swap n m by alternating c2 and c3

    - If we dropped constructor [c4] from the definition of [R],
      would the set of provable propositions change?  Briefly (1
      sentence) explain your answer.

      Not really, since we can again use c1, c2, c3 and to obtain the
      same values.
...
Proof.
  ...
  - intros. induction H.
    ...
    + unfold fR in *. rewrite <- plus_n_Sm. apply f_equal. apply IHR.
    + unfold fR in *. simpl in *. inversion IHR. rewrite <- plus_n_Sm in H1.
      apply (S_injective _ _ H1).
    + unfold fR in *. rewrite plus_comm. apply IHR.
  - intros. unfold fR in *. generalize dependent n. generalize dependent o.
    induction m.
    + induction o.
      * intros. simpl in H. rewrite H. apply c1.
      * intros. simpl in *. rewrite H. apply c3. apply IHo. reflexivity.
    + intros. rewrite <- H. rewrite plus_comm. rewrite <- plus_n_Sm.
      apply c2. apply IHm. rewrite plus_comm. reflexivity.
Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | subseq_cons1 : forall xs ys y, subseq xs ys -> subseq xs (y :: ys)
  | subseq_cons2 : forall xs ys x, subseq xs ys -> subseq (x :: xs) (x :: ys).

Theorem subseq_rel : forall (xs : list nat), subseq xs xs.
Proof.
  intros. induction xs.
  - apply subseq_nil.
  - apply subseq_cons2. apply IHxs.
Qed.
...
Proof.
  ...
  - simpl. apply subseq_cons2. apply IHsubseq.
Qed.

Theorem subseq_trans : forall l1 l2 l3, subseq l1 l2 -> subseq l2 l3 -> subseq l1 l3.
Proof.
  intros. 
  generalize dependent H. generalize dependent l1.
  induction H0.
  - intros. inversion H. apply subseq_nil.
  - intros. apply subseq_cons1. apply (IHsubseq _ H).
  - intros. inversion H.
    + apply subseq_nil.
    + apply subseq_cons1. apply (IHsubseq l1 H3).
    + apply subseq_cons2. apply (IHsubseq xs0 H3).
Qed.
...
Proof.
  ...
  - intros. generalize dependent s1. induction s2.
    ...
    + simpl. intros. inversion H. pose proof (IHs2 _ H4).
      rewrite H5. inversion H3. reflexivity.
  - intros. generalize dependent s1. induction s2.
    + simpl. intros. rewrite H. apply MEmpty.
    + simpl. intros. rewrite H. assert (x :: s2 = [x] ++ s2) by reflexivity.
...
Proof.
  ...
  - intros. induction re.
    ...
    + inversion H.
      pose proof (andb_true_elim2 _ _ H1).
      rewrite andb_commutative in H1.
      pose proof (andb_true_elim2 _ _ H1).
      pose proof (IHre1 H2).
      pose proof (IHre2 H0). destruct H3. destruct H4.
      exists (x ++ x0). apply (MApp _ _ _ _ H3 H4).
    + simpl in H. rewrite orb_true_iff in H. destruct H.
      * pose proof (IHre1 H). destruct H0. exists x. apply (MUnionL _ _ _ H0).
      * pose proof (IHre2 H). destruct H0. exists x. apply (MUnionR _ _ _ H0).
    + exists []. apply MStar0.
Qed.
...
Proof.
  ...
  - pose proof (IHexp_match2 Heqre'). destruct H1. destruct H1.
    exists (s1 :: x). split.
    + simpl. rewrite H1. reflexivity.
    + intros. inversion Heqre'. destruct H3.
...
(* aux lemma *)
Lemma app_nil : forall {X : Type} (l1 l2 : list X), l1 ++ l2 = [] -> l1 = [] /\ l2 = [].
Proof.
  induction l1.
  - intros. simpl in H. split. reflexivity. assumption.
...
(* Erh, OK, I probably missed something or maybe this proof is just huge? *)
Proof.
  ...
  - simpl. intros. rewrite app_length in H.
    pose proof (Nat.add_le_cases _ _ _ _ H). 
    destruct H0.
    + destruct (IH1 H0). destruct H1. destruct H1. destruct H1. destruct H2.
      exists x. exists x0. exists (x1 ++ s2). split.
      * rewrite H1. rewrite <- 2!app_assoc. reflexivity.
      * {split.
         - assumption.
         - intros. rewrite 2!app_assoc. apply MApp.
           + rewrite <- app_assoc. apply (H3 m).
           + apply Hmatch2.
        }
    + destruct (IH2 H0). destruct H1. destruct H1. destruct H1. destruct H2.
  ...
  - simpl. intros. destruct (plus_le _ _ _ H).
    destruct (IH H0). destruct H2. destruct H2. destruct H2. destruct H3.
    exists x. exists x0. exists x1. split.
    + assumption.
    + split.
      * assumption.
      * intros. apply MUnionL. apply (H4 m).
  - simpl. intros. destruct (plus_le _ _ _ H).
    destruct (IH H1). destruct H2. destruct H2. destruct H2. destruct H3.
    exists x. exists x0. exists x1. split.
    + assumption.
    + split.
      * assumption.
      * intros. apply MUnionR. apply (H4 m).
  - simpl. intros. inversion H.
  - intros. simpl in *. rewrite app_length in H. 
    induction s1.
    + intros. simpl in H. 
      destruct (IH2 H). destruct H0, H0, H0, H1.
      exists x. exists x0. exists x1. split.
    ...
    + intros. simpl in *. 
      ...
      * {split.
         ...
         - intros. simpl. induction m.
           + simpl.  apply Hmatch2.
           + simpl. inversion IHm.
             * simpl. rewrite <- app_assoc. rewrite <- H1. rewrite app_nil_r.
               apply MStar1. apply Hmatch1.
             * {rewrite <- app_assoc. rewrite <- H0. rewrite app_assoc.
                replace (x :: (s1 ++ s0) ++ s3) with ((x :: s1) ++ (s0 ++ s3)).
                - apply MStarApp.
                  + apply Hmatch1.
                  + apply MStarApp. apply H2. apply H3.
...
Proof.
  ...
  - intros. destruct H.
    + reflexivity.
    + unfold not in *. contradiction (H H0).
  - intros. rewrite H0 in H. inversion H. assumption.
Qed.
...
| pal_nil : forall X, pal X []
| pal_sing : forall X x, pal X [x]
| pal_cons : forall X x xs, pal X xs -> pal X (x :: (xs ++ [x])).

Arguments pal {X} _.
Arguments pal_nil {X}.
Arguments pal_sing {X} _.
Arguments pal_cons {X} _ _ _.
...
Theorem pall_rev : forall X (l : list X), pal l -> l = rev l.
Proof.
...
Theorem palindrome_converse_len : forall X (l : list X),
    l = rev l -> pal l.
Proof.
  intros. induction l.
  - apply pal_nil.
...
Theorem palindrome_converse_len : forall X n (l : list X),
    length l <= n -> l = rev l -> pal l.
Proof.
  induction n.
  - simpl. intros. inversion H. destruct l.
    + apply pal_nil.
    + inversion H2.
  - intros. induction l.
    + apply pal_nil.
...
Inductive in_order : forall X (l1 l2 l3 : list X), Prop :=
| in_ord_nil : forall X, in_order X [] [] []
| in_ord_cons_1 : forall X xs ys zs x, in_order X xs ys zs -> in_order X (x::xs) ys (x::zs)
| in_ord_cons_2 : forall X xs ys zs y, in_order X xs ys zs -> in_order X xs (y::ys) (y::zs).

Arguments in_order {X} _ _ _.
Arguments in_ord_cons_1 {X} _ _ _ _ _.
Arguments in_ord_cons_2 {X} _ _ _ _ _.

Theorem in_order_spec: forall X (test: X -> bool) (l1 l2 l : list X),
    in_order l1 l2 l ->
    All (fun a => test a = true) l1 ->
    All (fun a => test a = false) l2 ->
    filter test l = l1.
Proof.
  intros. induction H.
  - reflexivity.
  - simpl. inversion H0. rewrite H2. apply f_equal. apply IHin_order.
    + apply H3.
    + apply H1.
  - simpl. inversion H1. rewrite H2. apply IHin_order.
    + apply H0.
    + apply H3.
Qed.
...
Inductive NoDup : forall (X : Type) (l : list X), Prop :=
  | nodup_nil  : forall X, NoDup X []
  | nodup_cons : forall X x l, ~(In x l) -> NoDup X (x :: l).

(** Finally, state and prove one or more interesting theorems relating
    [disjoint], [NoDup] and [++] (list append).  *)

(* aux lemma we will need below *)
Lemma in_l_or_r : forall X (l r : list X) x, In x (l ++ r) -> In x l \/ In x r.
Proof.
  intros. induction l.
  - simpl in H. right. assumption.
  - simpl in H. destruct H.
    + left. rewrite H. unfold In. left. reflexivity.
    + destruct (IHl H).
      * left. unfold In. right. apply H0.
      * right. assumption.
Qed.

Theorem disj_app_NoDup : forall X (l1 l2 : list X),
    NoDup X l1 -> NoDup X l2 ->
    disjoint X l1 l2 -> NoDup X (l1 ++ l2).
Proof.
  unfold disjoint. intros. induction H.
  - simpl. apply H0.
  - simpl. apply nodup_cons. unfold not in *. intros.
    destruct (H1 x). apply H3.
    + unfold In. left. reflexivity.
    + destruct H0.
      * simpl in *. apply H. rewrite app_nil_r in H2. apply H2.
      * {destruct (in_l_or_r _ _ _ _ H2).
         - exfalso. apply (H H5).
         - apply H5.
        }
Qed.
...
| nostut_nil  : nostutter []
| nostut_sing : forall x, nostutter [x]
| nostut_cons : forall x' x xs, nostutter (x :: xs) -> (x <> x') ->
                  nostutter (x' :: x :: xs).
...
Proof.
  intros. induction l.
  - unfold In in H. exfalso. assumption.
  - unfold In in H. destruct H.
    + rewrite H. exists []. exists l. reflexivity.
    + simpl in H. destruct (IHl H) as [l1']. destruct H0 as [l2'].
...
Inductive repeats {X:Type} : list X -> Prop :=
  | repeats1 : forall x l, In x l -> repeats (x :: l)
  | repeats2 : forall x l, repeats l -> repeats (x :: l).
...
Proof.
   ...
   - intros. unfold lt in H0. simpl in H0. inversion H1.
   - intros.  destruct (H (In x l1')).
     + apply repeats1. apply H2.
     + apply repeats2. destruct (H (In x l2)).
