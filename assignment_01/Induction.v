...
Destruct simply pattern matches on the argument, and does case analysis.
It does not introduce the induction hypothesis.

Induction does case analysis and introduces the induction hypothesis into
the scope.
*)
...
Lemma mult_plus_expand : forall m n, m * (S n) = m + m * n.
Proof.
  intros. rewrite plus_comm. induction m.
  - reflexivity.
...
Proof.
  ...
  - simpl. rewrite <- IHn. rewrite mult_plus_expand. reflexivity.
Qed.
...
- reflexivity.
- simpl. rewrite <- plus_n_O. reflexivity.
- simpl. rewrite <- 2!plus_n_O.
  rewrite 2!plus_n_Sm. rewrite IHb. symmetry. rewrite <- plus_n_Sm.
  reflexivity.
Qed.
...
Theorem nat_bin_inj : forall n, bin_to_nat (nat_to_bin n) = n.
Proof.
  intros. induction n.
  - reflexivity.
...
(*
   (b) We cannot do this, because the same natural number can have
       multiple representations as binary numbers, using the definition
       we have created. e.g. zero can be [T (T (T Z)))] or simply [T Z].
*)

Definition norm_t (b: bin) : bin :=
  match b with
  | Z => Z
  | b' => T b'
  end.
...
Fixpoint normalize (b : bin) : bin :=
  match b with
  | Z => Z
  | T b' => norm_t (normalize b')
  | I b' => incr (norm_t (normalize b'))
  end.

Lemma nat_bin_t : forall b, 2 * bin_to_nat b = bin_to_nat (norm_t b).
Proof.
  induction b.
  - reflexivity.
  - reflexivity.
...
Lemma n_plus_n_2n : forall n, n + n = 2 * n.
Proof. intros. induction n.
       - reflexivity.
       - simpl. rewrite <- plus_n_Sm. rewrite IHn. rewrite <- plus_n_Sm.
         reflexivity.
Qed.

Lemma nat_to_bin_norm_t : forall n, nat_to_bin (n * 2) = norm_t (nat_to_bin n).
Proof.
  induction n.
  - reflexivity.
  - simpl. rewrite IHn. cut (forall b, incr (incr (norm_t b)) = norm_t (incr b)).
    + intros. apply H.
    + intros. induction b.
      * reflexivity.
      * reflexivity.
...
Theorem norm_b_inverse_nat : forall b, nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  induction b.
  - reflexivity.
  - simpl.  rewrite <- plus_n_O. rewrite <- IHb.
    rewrite n_plus_n_2n. rewrite mult_comm.
    rewrite nat_to_bin_norm_t. reflexivity.
  - simpl. rewrite <- plus_n_O. rewrite <- IHb.
    rewrite n_plus_n_2n. rewrite <- nat_to_bin_norm_t.
